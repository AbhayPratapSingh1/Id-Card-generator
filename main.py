...
class main(Handler):
    def __init__(self, data):
        ...
        self.backgroundPath = "User_Data_Place_Here/background.jpg"
        self.demoPhoto = "User_Data_Place_Here/1.png"
    ...
    def mergeField(self, image, h, h2, w, w2, to):
        if to == "img" :
            ...
            if self.currentField == -1 and self.mode == 3 :
            ...
            if self.imgField != -1 and self.mode == 3 :
                for i in range(0, h2-h):
                    for j in range(0,w2-w):
                        if not(image[i,j, 0] == 255 and image[i, j, 1] == 255 and  image[i,j, 2] == 255) :
    ...
    def createFieldBox(self, x=30, y=30, h=40, w=100):
        self.currentField =  fieldBox(x=x, y=y, h=h, w=w, i = 1) # new object is creted 
    

# this is for reading the data from the xlsx and take out the top column of the table
dataPath = "User_Data_Place_Here/data.xlsx"
dataHandler = dataManager(dataPath)
header = [ i.upper() for i in  dataHandler.getHeader()]

createModel = main(data = header)
print(createModel.fieldDetails)
